  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // 6087h is read-only on the verified driver.
  // Do not write torque slope here.
  ...
  const rclcpp::Duration & period)
{
  ...
  // Verified RPDO0 current mapping:
  // 6071h:01 left I16 + 6071h:02 right I16.
  struct can_frame frame {};
  frame.can_id = 0x200u + node_id_;
  frame.can_dlc = 4;
  ...
  const double dt)
{
  ...
  constexpr double BREAKAWAY_STEP_A  = 0.05;
  constexpr double BREAKAWAY_MAX_A   = 1.00;
  ...
  if (!state.tracking) {
    ...
    // Increase target by 0.05 A every 200 ms.
    if (state.breakaway_count > 0 &&
    ...
    if (state.start_count >= START_SAMPLES) {
...
{
  const uint32_t sdo_id = 0x600u + node_id_;
  const uint32_t rpdo0_cob = 0x200u + node_id_;
  ...
  // Verified current-command path:
  //
  // RPDO0  COB-ID = 0x200 + node
  // node 1 -> 0x201
  //
  ...
  if (!send_u32(
      0x1400, 0x01,
      0x80000000u | rpdo0_cob)) {
    return false;
  }

  if (!send_u8(0x1600, 0x00, 0)) {
    return false;
  }
